    ...
    for attempt in range(1, max_retries + 1):
        try:
            with uproot.open(file, timeout=timeout_s) as f:
                ...
                selection = (met100) & (nMuon == 1) & (reference_trigger) & (filters)  & (recoil70)
...
if __name__ == "__main__":
    ...
    with gzip.open('/home/jhong/run3Monotop/decaf/analysis/metadata/2022_for_recoil_trigger.json.gz', 'rt') as f:
        data = json.load(f)
    ...
    plt.legend()
    plt.grid()
    plt.savefig('trigger_efficiency_22pre_mc.png')

    with uproot.recreate("recoil_trigger_eff_22pre_mc.root") as f:
