...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    lumiMasks = {
        '2022pre': LumiMask("/cvmfs/cms.cern.ch/rsync/cms-nanoAOD/jsonpog-integration/POG/LUM/2022_Summer22/puWeights.json.gz"),
    }
    ...
    def __init__(self, year, xsec, corrections, ids, common):
        ...
        self.make_output = lambda: {
            ...
            'mupt': hist.Hist(
                hist.axis.StrCategory([], name='region', growth=True),
                hist.axis.Regular(38,250,1200, name='mupt', label='Leading Lepton Pt'),
                storage=hist.storage.Weight(),
            ),
            'mueta': hist.Hist(
                hist.axis.StrCategory([], name='region', growth=True),
                hist.axis.Regular(48,-2.4,2.4, name='mueta', label='Leading Lepton Eta'),
                storage=hist.storage.Weight(),
            ),
            'muphi': hist.Hist(
                hist.axis.StrCategory([], name='region', growth=True),
                hist.axis.Regular(64,-3.2,3.2, name='muphi', label='Leading Lepton Phi'),
                storage=hist.storage.Weight(),
            ),
            'l1pt': hist.Hist(
                hist.axis.StrCategory([], name='region', growth=True),
    ...
    def process_shift(self, events, shift_name):
        ...
        get_mu_hlt_id_sf         = self._corrections['get_mu_hlt_id_sf']
        ...
        if not isData:
            ...
            ids ={
                ...
                'allcr': get_mu_highpt_id_sf(self._year, abs(mu.eta), mu.pt),
            }
            ...
            isolation = {
                'nocr':  np.ones(len(events), dtype='float'),
                'pucr':  np.ones(len(events), dtype='float'),
                'allcr': get_mu_tight_iso_sf(self._year, abs(mu_tight.eta), mu_tight.pt),
            }
            ...
            isolation = {
                'nocr':  np.ones(len(events), dtype='float'),
                'pucr':  np.ones(len(events), dtype='float'),
                'allcr': get_mu_hlt_sf(self._year, abs(mu.eta), mu.pt),
            }
        ...
        selection.add('tiMuonpt', mu_tight.pt>53)
        ...
        for region in regions:
            ...
            if True:
                selection.add('test_selection', np.ones_like(mu_tight))
