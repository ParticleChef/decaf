...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def __init__(self, year, xsec, corrections, ids, common):
        ...
        self.make_output = lambda: {
            ...
            'purefj1pt': hist.Hist(
                hist.axis.StrCategory([], name='region', growth=True),
                hist.axis.Regular(38,250,1200, name='fj1pt', label='AK15 Leading Jet Pt (No corrections)'),
                hist.axis.Variable([0, self._TvsQCDwp[self._year], 1], name='TvsQCD', label='TvsQCD', flow=False),
                storage=hist.storage.Weight(),
            ),
            'fj1pt': hist.Hist(
...
#            & ak.all(fj.metric_table(pho_loose) > 1.5, axis=2)
        ...
        #fj_clean_mask = (
        #    ak.all(fj_good.metric_table(mu_loose) > 1.5, axis=2)
        #    & ak.all(fj_good.metric_table(e_loose) > 1.5, axis=2)
        #)
...
#            & ak.all(j.metric_table(pho_loose) > 0.4, axis=2)
        ...
        purefj = events.AK15PFPuppiJet
        purefj['isgood'] = isGoodAK15(purefj)
        purefj['isclean'] = (
            ak.all(fj.metric_table(mu_loose) > 1.5, axis=2)
            & ak.all(fj.metric_table(e_loose) > 1.5, axis=2)
#            & ak.all(fj.metric_table(pho_loose) > 1.5, axis=2)
        ...
        purefj_good = purefj[purefj.isgood]
        purefj_clean = purefj_good[purefj_good.isclean]
        leading_purefj = ak.firsts(purefj_clean)
